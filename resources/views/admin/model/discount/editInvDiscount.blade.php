<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header bg-primary">
			<div class="modal-title pull-left">DISCOUNT - EDIT</div>
			<button class="close pull-right" data-dismiss="modal">&times;</button>
		</div>
		<div class="modal-body">
			<form action="/admin/inventory/{{ $inventory->id }}/discount/{{ $discount->id }}/update" data-id="{{ $inventory->id }}" id="invDiscountForm">
				@csrf
				<table class="table" style="margin-bottom: 0;">
					...
					<tr>
						...
						<td>
							<div>
								<label><input type="radio" name="discount_type" @if($discount->is_amount) checked="checked" @endif id="discount_type" value="1">&nbsp; Amount</label> &nbsp;&nbsp;&nbsp;&nbsp;
								<label><input type="radio" name="discount_type" @if(!$discount->is_amount) checked="checked" @endif id="discount_type" value="0">&nbsp; Percent</label>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td>
							<div>
								<input type="number" name="discount_value" id="discount_value" class="form-control form-control-sm" style="width: 100px;height: 28px;" value="{{ $discount->is_amount ? $discount->amount : $discount->percent }}">
...
<script>
	$(function() {
		...
		$('#discount_name').select2({
			ajax: { url: '/admin/static/discounts', data: param => param, processResults: results => ({results}) }
		}).on('select2:select', e => {
